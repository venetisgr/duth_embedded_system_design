...
#define finalfilename "image_out.yuv" //output file name define
...
//since the mask is applied on each pixel
...
void gauss(){
	...
	int sum=0;    //used for normalization
    ...
    for(i=0;i<N;i++){
        y =-N/2;
        for(j=0;j<N;j++){ 
            g[i][j] =c*exp(-(pow (x,2)+pow(y,2))/(2*STD*STD)); // more efficient/faster to implement without pow
            sum =sum+g[i][j];
            y++;
        }
        x++;
    ...
    for( i=0;i<N;i++){
    	for( j=0;j<N;j++){ // can be combined with the above for-loop fusion
...
// image values range from 0 to 255 thus we need clipping
  ...
  for(i=0;i<im_rows;i++){
        for(j=0;j<im_collumns;j++){ // Imask=I-Is , Iunsharp=I+K*Imask
            image[i][j] =image[i][j] + K*(Imask[i][j]);
