...
#define finalfilename "image_out316.yuv" //output file name define
...
void convolution2D(){
  int i,j; //indexes //error when initialized inside for
    ...
    for( i=top+1;i<=im_rows+top;i++){
        for( j=1+left;j<=im_collumns+left;j=j+16){
            ...
            padded_image[i][j+8]= image[i-top-1][j+8-left-1];
            padded_image[i][j+9]= image[i-top-1][j+9-left-1];
            padded_image[i][j+10]= image[i-top-1][j+10-left-1];
            padded_image[i][j+11]= image[i-top-1][j+11-left-1];
            padded_image[i][j+12]= image[i-top-1][j+12-left-1];
            padded_image[i][j+13]= image[i-top-1][j+13-left-1];
            padded_image[i][j+14]= image[i-top-1][j+14-left-1];
            padded_image[i][j+15]= image[i-top-1][j+15-left-1]; 
        }  
    }
