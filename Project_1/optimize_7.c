...
#define finalfilename "image_out7.yuv" //output file name define
...
void final_steps(){
  int i,j;  //indexes //error when initialized inside for
  int temp_image[im_rows ][im_collumns]; 
// image values range from 0 to 255 thus we need clipping
  for(i=0;i<im_rows;i++){
        for(j=0;j<im_collumns;j++){ // Imask=I-Is , Iunsharp=I+K*Imask
            temp_image[i][j] =image[i][j] + K*(image[i][j]-conv_out[i][j]);
            if(temp_image[i][j]>255){image[i][j] =255;}// maximum pixel intensity is 255
            else if(temp_image[i][j]<0){image[i][j] =0;}// minimum pixel intensity is 0
        }
    }
}
//end final_steps
