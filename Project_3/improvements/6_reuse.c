...
#define finalfilename "image_out6big.yuv" //output file name define
  ...
  x =-N/2;
    ...
    for(i=0;i<N;i++){
        ...
        //for(j=0;j<N;j++){ //we avoided using the pow function, in order to make it
            g[i][0] =c*exp(-(x*x+y*y)/(2*STD*STD)); // more efficient/faster
            g[i][0] = g[i][0]/gauss_const;
            y++;
       // }
			g[i][1] =c*exp(-(x*x+y*y)/(2*STD*STD)); // more efficient/faster
            g[i][1] = g[i][1]/gauss_const;
            y++;
			
			g[i][2] =c*exp(-(x*x+y*y)/(2*STD*STD)); // more efficient/faster
            g[i][2] = g[i][2]/gauss_const;
            y++;
			
			g[i][3] =c*exp(-(x*x+y*y)/(2*STD*STD)); // more efficient/faster
            g[i][3] = g[i][3]/gauss_const;
            y++;
	   
			g[i][4] =c*exp(-(x*x+y*y)/(2*STD*STD)); // more efficient/faster
            g[i][4] = g[i][4]/gauss_const;
            y++;
...
void convolution2D(){
  int i,j; //indexes //error when initialized inside for
	...
	for(i=0; i<N; i++){  //loading the buffers we use for ??????
		for(j=0; j<padded_collumns; j++){
			reuse[i][j]= padded_image[i][j]	;		
		} 
	}
    ...
    for(x=0;x<im_rows;x++){//more efficient to read per row
        for( y=0;y<im_collumns;y++){  //kernell is applied on each pixel
            for(i=0;i<N;i++){ //center of the kernel is centered on the pixel we are reading
                    ...
                    conv_out[x][y]=conv_out[x][y]+( reuse[i][y]   *g_flipped[i][0] );
                    conv_out[x][y]=conv_out[x][y]+( reuse[i][y+1] *g_flipped[i][1]);
                    conv_out[x][y]=conv_out[x][y]+( reuse[i][y+2] *g_flipped[i][2]);
                    conv_out[x][y]=conv_out[x][y]+( reuse[i][y+3] *g_flipped[i][3]);
                    conv_out[x][y]=conv_out[x][y]+( reuse[i][y+4] *g_flipped[i][4]);
