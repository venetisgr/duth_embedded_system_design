...
#define finalfilename "image_out38big.yuv" //output file name define
        ...
        for( y=0;y<im_collumns;y++){  //kernell is applied on each pixel
            for(i=0;i<N;i++){ //center of the kernel is centered on the pixel we are reading
                for(j=0;j<N;j++){
					...
					//if(x<im_rows-5){
                   // conv_out[x][y]=conv_out[x][y]+( reuse[i][j+y] *g_flipped[i][j] );}
				//	else{conv_out[x][y]=conv_out[x][y]+( padded_image[x+i][j+y] *g_flipped[i][j] );}
